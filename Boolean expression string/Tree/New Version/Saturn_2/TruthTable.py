from CreateTree import CreateTree # create tree by list
from SplitExpression_V2 import Boolean_algebra # Split a Expression to a list
from ReadFile import Read

class TruthTable:
    def __init__(self, expr_str):
        self.expr = Boolean_algebra(expr_str)
        self.expr.split_expreesion()

        self.dict_operate = {}
        self.no_bin_operate = []
        self.MODE = "EXP" #EXP is only print expression form

        self.files = open("C:\\Users\\s6201012620279\\Software development\\050863\Tree\\New Version\\" + expr_str + ".txt", "w")
        for j in self.expr.operate_unique:
            if( j == "1" or j == "0"): # if it found "1" or "0" skip it
                continue
            
            self.files.write("|"+ "  " +str(j)+ (3-len(j))*" " +"|") #write on file
            print("|"+ "  " +str(j)+ (3-len(j))*" " +"|",end="")
            self.dict_operate[j] = 0
            self.no_bin_operate.append(j) # no "1" and "0" in this list

        self.bit = len(self.dict_operate)

    def create_table(self, tree_list):
        self.row("000", tree_list) # print head column
        self.MODE = "BIT" #BIT is only print result form
    ...
    def row(self, byte_str, tree_list):
        ...
        for oper,value in zip(self.no_bin_operate, byte_str):
            ...
            if(self.MODE == "BIT"):
                self.files.write("|"+ 2*" " +value+ 2*" " +"|") #write on file
                print( "|"+ 2*" " +value+ 2*" " +"|",end="" ) ##########
    ...
    def calculate(self, tree_list, given_list):
        ...
        while(True):
            ...
            if(   ((left_expr_checking or left_checking) and (right_checking or right_expr_checking) ) 
                ...
                # PRINT TABLE
                if( self.MODE == "EXP" ): # head column
                    self.files.write(f"|{new_tree_list[q]}|") #write on file
                    print(f"|{new_tree_list[q]}|",end="")
                elif( self.MODE == "BIT" ): # each values column

                    # code below is make line fit
                    if(len(new_tree_list[q])%2 == 0):
                        space_L = (len(new_tree_list[q]) -1)//2*" "
                        space_R = (len(new_tree_list[q]) )//2*" "
                    else:
                        space_L = (len(new_tree_list[q])-1) // 2*" "
                        space_R = (len(new_tree_list[q])-1) // 2*" "
                    self.files.write(f"|{space_L}{given_list[q]}{space_R}|") #write on file
...
if __name__ == "__main__":
    ...
    """testing_data_set = (  "!(1+0)" # index 0
                        ...
                        , "ABDFADPLKL + SDKMAKF") # index 5"""
    print(1)
    path = "C:\\Users\\s6201012620279\\Software development\\050863\Tree\\New Version\\Input.txt"
    testing_data_set = Read(path).getInput()
