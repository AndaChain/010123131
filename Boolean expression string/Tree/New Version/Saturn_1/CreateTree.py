...
import SplitExpression_V2 # Split a Expression to a list
...
class CreateTree:
    ...
    def resize(self):
        #new_number = int( math.log2(len(self.tree_list)+1) )
        #new_one = [""]* (2**(new_number+1) -1)
...
if( __name__ == "__main__" ):
    ...
    for test,index in zip(   Test_data_set, range (len(Test_data_set))   ):
        filter = Filter.check_bracket()
        split_expression = SplitExpression_V2.Boolean_algebra(test).split_expreesion()
        Test = CreateTree()
